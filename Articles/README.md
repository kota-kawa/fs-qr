...
- **`type: "article"` … 新着記事**
  ...
  公開から14日以内は **NEW** バッジが付く。公開前に独自性・正確性・
  サービスとの関連性を確認した記事だけを追加する。

> 記事を追加するときは、本文と主要言語の翻訳を完成させ、品質確認が終わるまで
> `indexable: False` にしておく。
...
AdSense 再審査中は、新規記事を作っただけでは検索・広告対象にしない。
本文の独自性、実画面の説明、運営者情報、最終確認日、導線を確認してから
...
`locales/<lang>/phrases/articles/<slug>.json` にも個別に追加する。
