...
3. startup で GeoIP 更新、Alembic upgrade、FSQR の期限切れ掃除、Note realtime の
...
- Note の接続数管理と room 更新・期限切れの pub/sub
- scheduler などの単一実行ロック

Redis 障害時の挙動は機能ごとに異なります。presence と Note realtime は限定的な
