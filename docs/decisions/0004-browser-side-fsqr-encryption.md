...
アップロード時は、サーバーのダウンロード認証に使う6桁のパスワードと、AES-GCM の
32バイト乱数鍵を別々に生成します。乱数鍵は `#key=` URL fragment にだけ保持して送信し、
サーバーへ送られる `download_password` は認証用パスワードだけにします。旧リンクの
`#pw=` およびサーバー描画パスワードは互換性のため復号側で受け付けます。
保存形式は `fsqr.encryption_mode` で識別し、既存レコードは `password` のまま維持します。
新しい `raw` レコードは復号鍵を含む共有 URL 経由だけを許可し、ID とパスワードの検索経路では
復号不能な状態でダウンロードを開始しないようにします。既存環境への列追加は Alembic
`20260919_0015_fsqr_encryption_mode` が担当します。
