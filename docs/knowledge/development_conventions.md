...
- レビュー用サブエージェント: 読み取り専用で動く汎用エージェント（Claude Code では Agent ツールの
  `general-purpose` に「ファイルを編集しない」と明示する。他の環境では同等の読み取り専用エージェント）
  を使います。渡すものは「スコープ」「差分（`git diff main...HEAD` など）」「チェックリスト
