...
- サブエージェントのモデル: レビュー、調査、実装分担のどの役割でも、Claude Code では Agent ツールの
  `general-purpose` にモデル `sonnet5`、effort `high` を指定し、Codex ではモデル `gpt-6-luna`、
  reasoning effort `max` を指定します。他の環境では同等の設定を使います。
- レビュー用サブエージェント: 読み取り専用とし、「ファイルを編集しない」と明示します。
